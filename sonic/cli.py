...
from sonic.ledger.db import DEFAULT_DB_PATH, init_db, get_conn, table_names
...
def _cmd_payment_link(args: argparse.Namespace) -> int:
    from sonic.razorpay_client.client import create_payment_link, fetch_payment_link
    ...
    print(f"  fixture  : sonic/razorpay_client/fixtures/payment_link.json")
    if link.get("id"):
...
def _cmd_bench(args: argparse.Namespace) -> int:
    import time
    from sonic.config.constants import BELIEF_TABLE, Cause
    from sonic.recover.policy import choose_smart
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="sonic", description=__doc__)
