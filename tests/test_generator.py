"""Week-1 acceptance tests: seeded generators + settlement substrate + persistence."""
...
def test_onetime_failures_reproducible():
    ...
    assert a == b  # same seed -> identical rows
    assert len(a) == 200
...
def test_cause_mix_reasonable_at_scale():
    ...
    for f in fails:
        counts[f["cause"]] += 1
    # insufficient_funds is the modal cause (weight 0.34); every cause appears.
...
def test_seed_ledger_persists_and_is_reproducible(tmp_path):
    ...
    try:
        ...
        assert s1 == s2  # deterministic summary
